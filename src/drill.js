...
const correctAllAudio = new Audio('/touch-kanji/mp3/correct1.mp3');
const correctAudio = new Audio('/touch-kanji/mp3/correct3.mp3');
const incorrectAudio = new Audio('/touch-kanji/mp3/incorrect1.mp3');
const stupidAudio = new Audio('/touch-kanji/mp3/stupid5.mp3');
...
function getGrade(str) {
  for (var grade=1; grade<gradeByKanjis.length; grade++) {
...
function getDictUrl(kanji) {
  var baseUrl = 'https://marmooo.github.io';
  var grade = getGrade(kanji);
  if (grade < 0) {
    return null;
  } else if (grade == 10) {
...
function fetchKanjiSVG(kanjiId) {
  return new Promise((resolve, reject) => {
    fetch(kanjivgDir + '/' + kanjiId + '.svg')
      .then(response => response.text())
      .then(text => {
        const parser = new DOMParser();
        return parser.parseFromString(text, 'text/html');
      }).then(html => {
        var kanji = html.getElementsByTagName('svg')[0];
...
function setScoringButton(problemBox, tegakiPanel, tehonPanel, objects, tegakiPads, word) {
  ...
  scoring.addEventListener('click', function() {
    ...
    getProblemScores(tegakiPanel, tehonPanel, objects, tegakiPads).then(scores => {
      ...
      var clearedKanjis = localStorage.getItem('touch-kanji');
      if (clearedKanjis) {
        var removed = false;
...
function setSignaturePad(object) {
  var canvas = object.parentNode.querySelector('canvas');
...
function setDict(tehonPanel, object, kanji) {
  var pos = parseInt(object.dataset.pos);
  var dict = tehonPanel.children[pos].shadowRoot.querySelector('#dict');
  var url = getDictUrl(kanji);
  if (url) {
    dict.href = url;
  } else {
...
function getInclusionCount(markerPad, markerContext, tegakiDatum, tehonData) {
  markerPad.clear();
  markerPad.fromData([tegakiDatum]);
  var tegakiImageData = markerContext.getImageData(0, 0, canvasSize, canvasSize);
  var tegakiData = tegakiImageData.data;
  for (var i=0; i<tegakiData.length; i++) {
    if (tehonData[i] != 0) {
      tegakiData[i] = 0;
    }
  }
  markerContext.putImageData(tegakiImageData, 0, 0);
  var imgData = markerContext.getImageData(0, 0, canvasSize, canvasSize);
  var inclusionCount = countNoTransparent(imgData.data);
...
function getKakuScores(tegakiData, object, kanjiId, kakusu) {
  ...
  for (var i=0; i<tegakiData.length; i++) {
    promises[i] = new Promise((resolve, reject) => {
      ...
      var data = markerContext.getImageData(0, 0, canvasSize, canvasSize).data;
      var tegakiCount = countNoTransparent(data);
      ...
      getTehonCanvas(object, kanjiId, kakusu, i+1).then(tehonCanvas => {
        ...
        var tehonImageData = tehonCanvas.getContext('2d').getImageData(0, 0, canvasSize, canvasSize);
        var tehonData = tehonImageData.data;
        var tehonCount = countNoTransparent(tehonData);

        var inclusionCount = getInclusionCount(markerPad, markerContext, tegakiDatum, tehonData);
...
function initQuery(mode) {
  ...
  fetchJsons(uniq(targetGrades)).then(data => {
    if (targetKanjis.length == 1) {
      var kanji = targetKanjis[0];
      var grade = targetGrades[0];
      var problems = [data[grade][kanji].shift()];
      problems = problems.concat(shuffle(data[grade][kanji]).slice(0, num));
    } else {
      var problems = [];
      targetKanjis.forEach((kanji, i) => {
        var grade = targetGrades[i];
        var candidates = data[grade][kanji].slice(1);
