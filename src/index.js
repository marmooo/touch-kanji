...
function getDictUrl(kanjiId) {
  var baseUrl = 'https://marmooo.github.io';
  var kanji = toKanji(kanjiId);
  var included = false;
  for (var i=0; i<gradeByKanjis.length; i++) {
    if (gradeByKanjis[i].includes(kanji)) {
      included = true;
      return baseUrl + '/kanji-dict/' + dirNames[i] + '/' + kanji + '/';
    }
  }
...
function testRemained() {
  ...
  location.href = `/touch-kanji/drill/?kanji=${target}`;
}
...
function deleteData() {
  localStorage.removeItem('touch-kanji');
  location.reload();
}
...
function generateDrill() {
  ...
  if (words && words.split('').some(word => w9.includes(word))) {
    location.href = `/touch-kanji/drill/?kanji=${words}`;
  }
}
...
function setProblems(obj) {
  ...
  for (var i=0; i<gradeByKanjis[grade].length; i++) {
    ...
    a.href = `/touch-kanji/drill/?kanji=${kanji}`;
    a.innerText = kanji;
    obj.appendChild(a);
  }
}
